...
* `db_cluster_version` - (Optional,ForceNew) Cluster version. Value options: 3.0.
* `db_cluster_category` - (Optional,ForceNew) Cluster category. Value options: Basic, Cluster.
* `db_cluster_network_type` - (Optional,ForceNew) Cluster network type. Value options: VPC.
...
* `create` - (Defaults to 30 mins) Used when creating the adb cluster (until it reaches the initial `Running` status). 
