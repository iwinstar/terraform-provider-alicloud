	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"db_cluster_network_type": {
				...
				ForceNew:     true,
				Optional:     true,
				ValidateFunc: validation.StringInSlice([]string{strings.ToUpper(string(Vpc))}, false),
				Default:      strings.ToUpper(string(Vpc)),
			},
			"db_node_class": {
				Type:     schema.TypeString,
				ForceNew: true,
...
func resourceAlicloudAdbClusterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	stateConf := BuildStateConf([]string{"Preparing", "Creating"}, []string{"Running"}, d.Timeout(schema.TimeoutCreate), 30*time.Minute, adbService.AdbClusterStateRefreshFunc(d.Id(), []string{"Deleting"}))
